...
class DelAutForm(QWidget):
    def __init__(self, db, parent=None):
        ...
        self.db = db
        self.setWindowTitle("Lista de Autobus")
    ...
    def delete_item(self, item_id):
        try:
            reply = QMessageBox.question(self, 'Eliminar Autobus', 
                                         '¿Estás seguro de que quieres eliminar este Autobus?', 
                                         QMessageBox.Yes | QMessageBox.No, QMessageBox.No)

            if reply == QMessageBox.Yes:
                query = "DELETE FROM autobus WHERE eco = %s"
                self.db.cursor.execute(query, (item_id,))
                self.db.connection.commit()
                QMessageBox.information(self, 'Éxito', 'Autobus eliminado correctamente', QMessageBox.Ok)
                self.load_data()  # Recargar los datos después de la eliminación

        except psycopg2.Error as e:
            self.db.connection.rollback()
            print(f"Error durante la eliminación del Autobus: {e}")
            QMessageBox.critical(self, 'Error', f'No se pudo eliminar el Autobus: {e}', QMessageBox.Ok)
        except Exception as e:
